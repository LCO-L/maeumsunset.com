...
function renderCampfires(){
  ...
  for(let i=0;i<3;i++){
    ...
    wrap.innerHTML = `<div class="campHeader">
        <div class="subtle">캠프파이어 ${i+1}</div>
    ...
    // intensity = likes/notes 기반 스케일 (1 ~ 2)
    ...
    // click handlers
    wrap.querySelector('.btn.ghost').onclick = ()=> openCamp(roomId);
    wrap.querySelector('.btn:not(.ghost)').onclick = ()=>{
      // write into this room directly (use journal value if any)
      const text = (journal?.value || "").trim();
      if(!text){ showToast("상단에서 내용을 입력한 뒤 저장하거나, 캠프 안에서 직접 써주세요."); openCamp(roomId); return; }
      const d = loadRoom(roomId);
      d.answers.unshift({ uid: state.uid, text, likes:0, ts: Date.now() });
      saveRoom(roomId, d);
      journal.value = ""; updateLengthMeter();
      showToast(`캠프파이어 ${i+1}에 저장했어요 🔥`);
      renderCampfires();
      openCamp(roomId);
    };
