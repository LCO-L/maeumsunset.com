...
async function init(){
  ...
  // Load state
  const saved = localStorage.getItem('sunset_state');
  if(saved){ state = {...state, ...JSON.parse(saved)}; }
  // Yesterday preview
  const ys = localStorage.getItem('sunset_yesterday');
...
function saveState(){
  localStorage.setItem('sunset_state', JSON.stringify(state));
}
...
$('#saveBtn').addEventListener('click', ()=>{
  ...
  // save content
  localStorage.setItem('sunset_last', text);
...
$('#shareBtn').addEventListener('click', ()=>{
  ...
  if(state.longCount < 3){
    showToast(`장문 ${3-state.longCount}회 더 작성하면 Campfire 공유 가능!`);
    return;
  }
...
function updateUI(){
  ...
  campfireGate.textContent = `Campfire 입장권: 장문 ${state.longCount}/3`;
  streakBadge.textContent = `연속 ${state.streak}일`;
}
...
document.getElementById('saveBtn').addEventListener('click', ()=>{
  ...
  localStorage.setItem('sunset_last', text);
  localStorage.setItem('sunset_last_date', new Date().toISOString().slice(0,10));
  $('#yesterdayPreview').textContent = text.slice(0, 20) + (text.length>20?'…':'');
  updateStreak();
...
const _oldSaveHandler = document.getElementById('saveBtn').onclick;
document.getElementById('saveBtn').onclick = ()=>{
  const st = currentPhase();
  if(!(st.c1 && st.c2 && st.c3)){
    showToast("각 단계 70자 이상 작성해주세요.");
    return;
  }
  ...
  if(totalLen < 80){
    ...
    addLogUnits(3);
    addScore(3.5); // stronger than single-field
    showToast("장작이 가득 쌓였습니다! ✨ (3단계 완료)");
    burst();
    state.longCount += 1;
  }
  // Save content (private, local)
  localStorage.setItem('sunset_last', text);
  localStorage.setItem('sunset_last_date', new Date().toISOString().slice(0,10));
  $('#yesterdayPreview').textContent = text.slice(0, 20) + (text.length>20?'…':'');
  updateStreak();
