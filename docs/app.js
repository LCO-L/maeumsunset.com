...
// touch mirror for saveBtn
document.getElementById('saveBtn').addEventListener('touchend', (e)=>{ e.preventDefault(); document.getElementById('saveBtn').click(); });

try{ document.addEventListener('touchstart', ()=>{}, {passive:true}); }catch(_){}


// --- Global event delegation for stubborn tap environments ---
document.addEventListener('click', (e)=>{
  const el = e.target.closest('button, .btn');
  if(!el) return;
  if(el.id==='saveBtn'){ e.preventDefault(); doSave && doSave(); }
  else if(el.id==='shareBtn'){ e.preventDefault(); doShare && doShare(); }
  else if(el.id==='nextQ'){ e.preventDefault(); doNextQ && doNextQ(); }
  else if(el.id==='probeBtn'){ e.preventDefault(); doProbe && doProbe(); }
  else if(el.id==='installBtn'){ e.preventDefault(); document.getElementById('installBtn').click(); }
  else if(el.id==='shareInBtn'){ e.preventDefault(); document.getElementById('shareInBtn').click(); }
  else if(el.id==='dockOk'){ e.preventDefault(); document.getElementById('dockOk').click(); }
});
