...
document.addEventListener('DOMContentLoaded', () => {
  ...
  canvas = document.getElementById('campfire');
  ctx = canvas?.getContext && canvas.getContext('2d');

  P1 = document.getElementById('phase1');
  P2 = document.getElementById('phase2');
  P3 = document.getElementById('phase3');
  phaseLenHint = document.getElementById('phaseLenHint');
  ...
  if(saveBtn){
    ...
    saveBtn.addEventListener('click', ()=>{
      ...
      updateStreak();
      journal.value = '';
      if(lengthHint) lengthHint.textContent = '0자';
      updateUI();
      drawFire();
    });
  ...
  if(shareInBtn && journal){
    shareInBtn.addEventListener('click', ()=>{
      const sample = "외부에서 공유된 텍스트가 여기에 들어옵니다.";
      journal.value = sample;
      if(lengthHint) lengthHint.textContent = sample.length + "자";
      showToast("공유로 열렸습니다.");
    });
  }

  const dockOk = document.getElementById('dockOk');
  if(dockOk){
    dockOk.addEventListener('click', ()=>{
      const banner = $('#dockBanner');
      if(banner) banner.classList.remove('show');
    });
  }
